...
class MessageController extends BaseController
{
    ...
    public function chat_room()
    {
       ...
       // dd($all_channel_id);
        ...
        if(count($all_channel)>0){
            ...
            $save_data=[];
            foreach($all_channel as $all_channels)
            {
              if($all_channels->user_logo_2!=null)
              {
                $logo = $all_channels->user_logo_2;
              }
              else
              {
                $logo = 'default.png';
              ...
              $save_data[]=[
              'user_id_1' => $all_channels->user_id_1,
              'user_id-2' => $all_channels->user_id_2,
              'dep_name' => $all_channels->dep_name,
              'dep_company_name' => $all_channels->dep_company_name,
              'user_logo' =>  asset('/companyLogo/'.$logo),
              // 'user_logo_2' => $all_channels->user_logo_2,
              // 'user_name_1' => $all_channels->user_name_1,
              'user_name' => $all_channels->user_name_2, ];
            }
        } 
        $status = array(
          'error' => false,
          'all_channel' => $save_data,
    ...
    public function setChatNotification($sender_id,$receiver_id,$chat_id)
    {
      ...
      $sendNotification = $this->sendNotificationSupplier($firebaseToken, $last_title_sup, $last_body_sup, $last_link_sup,$noti_save->id); 
            ...
            Mail::send('mail.chat_room_notification', 
            [ 'r_user'=>$receiver_users ,'s_user'=>$sender_user,'last_link_sup'=>$last_link_sup], 
            function ($m) use ($receiver_users) 
            {
