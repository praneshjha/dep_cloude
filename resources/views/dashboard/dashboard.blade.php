...
@section('content')
      ...
      <div class="container-fluid">
        <div class="row">
          ...
          <div class="popup" id="popup">
                  <span class="close" onClick="removeCon()">&times;</span>
                  <div>Search departures by destinations, tags, and supplier’s  name. Next, select the  departure type and choose the travel date from the drop-down menu.</div>
             </div>
            <div class="card p-3">
              <form class="searchPage">
                ...
                <div class="upcomingDepa">
                @foreach($destinationFind as $findDepa)
                <div class="upcoming">
                   <div class="secDiv">
                       <a href='{{ url("dashboard") }}?type=11&keyword={{ $findDepa->dest_name}}'>#{{$findDepa->dest_name}}</a>
                    </div>
                  </div>
               @endforeach
               </div>
  ...
  <div class="container-fluid">
    <div class="row">
      ...
      <?php
       ...
       {  
        ...
        $new_dep = DB::table('departures')
          ...
          ->count();
      ?>
      <div class="col-md-2 gridFav d-flex" id="gridFav_pop_{{$i}}">
        <div class="card p-0 m-0 w-100">
          ...
          <div class="card-body p-1 pl-2">
            ...
            <a href="javascript:void(0);" class="showFavPopclick" id="pop_{{$i}}"><i class="mdi mdi-comment-plus-outline"></i></a>
          </div>
           <div class="HoverFav">
                <div class="d-flex"> 
                  <picture>
                    @if($supplier->logo != "")
                    <img src="{{ asset('companyLogo/' . $supplier->logo) }}" width="40" class="mr-2">
                    @else
                    <img src="{{ asset('assets1/images/supplier_icon.png')}}" width="40" class="mr-2">
                    @endif
                  </picture>                   
                    <div> {{ $supplier->company_name }} is giving best services <strong>@foreach($details as $det) <a href='{{ url("dashboard") }}?type=13&keyword={{ $supplier->company_name}}&destination={{ $det->from }}' class="supplier-dest"><i class="mdi mdi-map-marker-radius"></i> {{ $det->from }}</a> @endforeach</strong> in these destinations.</div>
                </div>
            </div>
        </div>       
      </div>
      <?php $i++; } ?>
    @endforeach
    @foreach($fav_pkg_select as $pkg_show)
    <div class="col-md-2 gridFav d-flex">
        <div class="card p-0 m-0 w-100">
          ...
          <div class="card-body p-1 pl-2">
            ...
            <div><span>Date:</span> {{date('d M, Y', strtotime($pkg_show->start_date))}}</div>
            <div>Seats (A/T) : <strong >@if($pkg_show->available_seat == "" ) NA @else{{ $pkg_show->available_seat}}@endif</strong> / <span>{{ $pkg_show->total_seat }}</span></div>
        </div>       
      </div>
    </div>
    @endforeach
    ...
    <div class="col-md-2 gridFav d-flex" style="align-self:center;">
      <div class="card p-0 m-0 ">
        <div class="card-body p-1 pl-2">
          <h5 class="card-title">
            <a href="{{route('profile')}}#favSupplier"><i class="mdi mdi-plus-box"></i> Show More</a>
    ...
    <div class="container-fluid">
      ...
      <div class="row">
        <div class="col-md-9">
          ...
          <!-- {{-- $departures->appends(request()->input())->links() --}} -->
          {{-- $departures->appends(Request::all())->links() --}}
...
.upcomingDepa .upcoming .secDiv a{
  font-size: 13px;
  text-align: center;
  cursor: pointer;
  color: #6b6363;
...
.upcomingDepa .upcoming .secDiv a:hover {
    color: #1758cb;
}
