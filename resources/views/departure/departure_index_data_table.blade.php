...
<div class="col-md-12">
    <div class="row mt-3">
                    ...
                    <table class="table table-striped table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{$departure->from}}<span style="color:#69214c"> - </span>{{$departure->ending_at}}<span style="color:#69214c"> - </span>{{$departure->return_to}}</td>
                                <td>
                                    Total: <span class="text-danger">{{$departure->total_seat}} </span>
                                    Book: <span class="text-danger">{{$departure->book_sum}}</span>
                                    Hold: <span class="text-danger">{{$departure->hold_sum}}</span>
                                </td>
                                <td>
                                    ...
                                    @if($departure->start_date >= $today)
                                        @if($departure->status == 1 )
                                            @if($departure->approve == 1)
                                                ...
                                                @else
                                                   <span class="text-danger"> SOLDOUT </span>
                                                @endif
                                            @else
                                ...
                                <td>
                                    @if($departure->status == 0 )
                                       <a href="javascript:void(0);" @if($departure->inclusion < 1) onClick="alert('Please fill the inclusion section before publishing the departure')" @elseif($departure->DeparturePrice < 1) 
                                       onClick="alert('Please ensure that the minimum price before publishing the departure')" @elseif($departure->payment_schedule < 1 ) onClick="alert('Please fill the payment schedule section before publishing the departure')"
                                       ...
                                       @else data-toggle="modal" data-target="#myModal{{$departure->id}}" @endif data-id="{{ $departure->id }}" data-status="{{ $departure->status }}" title="Publish Departure" ><i class="fa fa-upload"></i> </a>
                                    @else
                                    ...
                                    @endif
                                    @can('departure_view', $permission)
                                        <a href="{{route('departure_details',$departure->id)}}" class="" title="View Departure" style=""><i class="fa fa-eye"></i></a>
                                    @endcan
                                    @can('departure_edit', $permission)
                                        <a href="{{route('departure_edit',$departure->id)}}" class="" style="" title="Edit Departure"><i class="fa fa-edit"></i></a>
                                    @endcan
                                    @can('departure_hold', $permission)
                                        @if($departure->start_date >= $today)
                                            @if($departure->approve == 1 )
                                                <a href="" class="" data-toggle="modal" data-target="@if(($hold < $date))#modal{{$departure->id}}@endif" title="Hold Units" style=""><i class="fas fa-pause"></i></a>
                                            @endif
                                        @endif
                                    @endcan
                                    ...
                                    @if($departure->approve == 1 || $departure->company_publish == 1)
                                        @if($departure->start_date >= $today)
                                            <a href="" class="" data-toggle="modal" data-target="@if((($departure->total_seat)-($departure->hold_sum + $departure->book_sum )) != 0) .bd-example-modal-sm{{$departure->id}}b @endif" title="Book Units" style=""><i class="fas fa-clipboard-check"></i></a>
                                        @endif
                                    @endif
                                    @can('departure_booking_history', $permission)
                                        @if($departure->approve == 1 || $departure->company_publish == 1)
                                            <a href="{{route('departure_booking_history', ['id' => $departure->id])}}" class="" title="Booking History" style=""><i class="fas fa-calendar-alt"></i></a>
                                        @endif
                                    @endcan
                                    @if($departure->approve == 1)
                                        {{--<a href="" class="" data-toggle="modal" class="copyDepartue{{$row->id}}C" data-target=".bd-example-modal-sm{{$departure->id}}C" title="Clone Departure"><i class="fa fa-clone" aria-hidden="true"></i>
                                        </a>--}}
                                        <span class="dropdown cloneDepDropclick{{$departure->id}}" id="">
                                            <a href="#" class="btn btn-sm btn-clean btn-icon btn-icon-md " data-toggle="dropdown" id="cloneDepDrop{{$departure->id}}" onclick="DepCloneClick({{$departure->id}})" aria-expanded="true" title="Clone Departure">
                                            <i class="fa fa-clone" aria-hidden="true" style="color:#395eba"></i>
                                            </a>
                                            <div class="dropdown-menu dropdown-menu-right">
                                                <a href="" class="dropdown-item copyDepartue{{$departure->id}}C" data-toggle="modal"  data-target=".bd-example-modal-sm{{$departure->id}}C" title="Clone Departure" style="color:#000;"> Clone to a Departure
                                                </a>
      ...
      <div class="col-md-12 pagiNate">{{$departures->withQueryString()->links()}}
      </div>
    </div>
</div>
@section('footerSection')
    ...
    <script type="text/javascript">
        ...
        //   if (confirm("you can not publish departure .please check departure details and fill all step."))
        //   var id = $(this).data("id");
        //   var status = $(this).data("status");
        ...
        //   }
        // });
        //alert('you can not publish departure .please check departure details and fill all step.');
    </script>
    ...
    @foreach( $departures as $row )
        ...
        <!----End Modal-->
        <div class="modal fade bd-example-modal-sm{{$row->id}}C" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true" style="width:367px; min-height:100vh">
                    ...
                    <form id="myCopyForm{{$row->id}}">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <input type="hidden" class="form-control" id="dep-id" name="dep_id" value="{{($row->id)}}">
                                <input type="text" class="form-control" id="title_{{($row->id)}}" name="title" value="{{$row->title}} Copy">
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group date">
                                    <input type="text" class="form-control pull-right start_date{{$row->id}} fromdate" name="start_date" id="start_date{{$row->id}}">
                                    <div class="input-group-prepend">
                                        <span class="input-group-text" id="calendar{{$row->id}}"><i class="fa fa-calendar calendar{{$row->id}}" aria-hidden="true"></i></span>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="exampleFormControlInput1">Total Units<span class="validationError" id="total_seaterror_{{$row->id}}"></span></label>
                                <input type="text" class="form-control" name="total_seat" value="{{$row->total_seat}}" id="total_seat_{{$row->id}}">
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button class="btn btn-secondary" data-dismiss="modal" id=""><i class="flaticon-cancel-12"></i> Close</button>
                            <button type="submit" class="btn btn-primary" id="copyDeparture_{{$row->id}}">Clone Departure</button>
                            <img src="{{ asset('images/loader.gif') }}" id="gif_{{$row->id}}" style="width: 12%; visibility: hidden;">
                                            ...
                                            <div class="col-md-2">
                                                <div class="form-group">
                                                    <input type="text" class="form-control" id="" name="hotel_name[]"
                                                           value="{{$require->hotel_name}}" readonly>
                                                </div>
                                            </div>
                                            <div class="col-md-2">
                                                <div class="form-group">
                                                    <input type="text" class="form-control" id="" name="hotel_type[]"
                                                           value="{{$require->hotel_type}}" readonly>
                                                </div>
                                            </div>
                                            <div class="col-md-2">
                                                <div class="form-group">
...
@foreach($departures as $departure)
     <!----Modal-->
     <div class="modal fade" id="myModal{{$departure->id}}" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-md" role="document">
 <!-- <div class="modal" id="myModal{{$departure->id}}" style="position:fixed; top: 48px;">
      ...
      <div class="modal-content" style="min-height: 152px; width: 50%;">
        ...
        <div class="modal-body">
            ...
            <div class="form-check">
              <input type="radio" class="form-check-input" id="publish2" name="publish{{$departure->id}}" value="2" checked>
              <label class="form-check-label" for="materialChecked">Publish for all</label>
            </div>      
            @if($departure->company_publish == 0)
            <div class="form-check">
              <input type="radio" class="form-check-input" id="publish1" name="publish{{$departure->id}}" value="1">
        ...
        <div class="modal-footer" style="text-align: right;">
          <button  class="btn btn-primary btn-sm"  id="departure_published{{$departure->id}}" >Submit</button>
