            ...
            <div class="row">
                <div class="col-12">
                    <div class="card-box">
                            ...
                            <div class="col-xl-6 pl-md-0 departureDetail">
                                <div class=" d-flex justify-content-between align-items-center">
                                    ...
                                    <div class="btn-group w-100 d-flex justify-content-end">
                                        <!-- <p class="text-right"><h4 style="margin: 5px 0;">&nbsp; Last Updated: </h4> <span class="LastUpdated"> {{date('d M, Y H:i A', strtotime($departure_details->last_updated_dep))}}</span></p> -->
                                        @if(auth::user()->main_user_type==2)
                                            @if($departure_details->start_date >= date('Y-m-d'))
                                                {{--<a href="{{url('/departure-basic-details-edit/'.$departure_details->id)}}" class="btn btn-info btn-sm pull-right ml-2">Edit Departure</a> --}}

                                                @if($departure_details->approve == 1)
                                                    @if($departure_details->start_date >= $today)
                                                        <a href="" class="btn btn-primary btn-sm pull-right ml-2" data-toggle="modal"
                                                           data-target="@if((($departure_details->total_seat)-($departure_details->hold_sum + $departure_details->book_sum)) > 0).bd-example-modal-sm{{$departure_details->id}} @endif">Book Units</a>
                                                        <a href="" class="btn btn-info btn-sm pull-right ml-2" data-toggle="modal" data-target="@if($hold_till->hold_till < $date).bd-example-modal-sm @endif">Hold Units</a>
                                                    @endif
                                                @endif
                                            @endif
                                        @else
                                            @if($departure_details->start_date >= $today)
                                                <a href="" class="btn btn-primary btn-sm pull-right ml-2" data-toggle="modal"
                                                   data-target="@if((($departure_details->total_seat)-($departure_details->hold_sum + $departure_details->book_sum)) > 0).bd-example-modal-sm{{$departure_details->id}} @endif">Book Units</a>
                                                <a href="" class="btn btn-info btn-sm pull-right" data-toggle="modal" data-target="@if($hold_till->hold_till < $date).bd-example-modal-sm @endif">Hold Units</a>
                                            @endif
                                        @endif
                                    </div>
                                </div>
                                <div>
                                    <h2 class="mb-1 text-capitalize" style="color: #093E8E;">{{$departure_details->title}}</h2>
                                </div>
                                <div class="mb-0 d-flex align-items-center justify-content-between position-relative">
                                    <div class="d-flex align-items-center">
                                        <div class="CompanyName">
                                            <a href="{{url('profile/'.$company_url)}}" class="userprofileName"> <span>{{$departure_details->departure_ownner}}</span></a>
                                            <div class="companyOverlay">
                                                <div class="companyDetailsBox">
                                                    <div class="topBox">
                                                        @if($company_details_all->logo == null)
                                                            <figure><img src="{{asset('images/company_icon.png')}}" alt="company-profile" class="img-fixed"></figure>
                                                        @else
                                                            <figure><img src="{{url('companyLogo')}}/{{$company_details_all->logo}} " alt="company-profile" class="img-fixed"></figure>
                                                        @endif
                                                        <div class="name">
                                                            <h5>{{$company_details_all->name}} {{$company_details_all->last_name}}</h5>
                                                            <h4>{{$company_details_all->company_name}}</h4>
                                                        </div>
                                                    </div>
                                                    <hr>
                                                    <div class="detailBox">
                                                        <p class="text-muted"><i class="fas fa-phone fa-rotate-90"></i> <a href="tel:{{$company_details_all->mobile}}">{{$company_details_all->mobile}}lol</a></p>
                                                        <p class="text-muted"><i class="fas fa-envelope"></i> <a href="mailto:{{$company_details_all->email}}">{{$company_details_all->email}}</a></p>
                                                        @if($company_details_all->website == null)

                                                        @else
                                                            <p class="text-muted"><i class="fa fa-globe"></i> <a href="{{$company_details_all->website}}">{{$company_details_all->website}}</a></p>
                                                        @endif
                                                        <p class="text-muted"><i class="fas fa-map-marker-alt"></i> @if($company_details_all->address == null)

                                                            @else
                                                                {{$company_details_all->address}}
                                                            @endif
                                                            <br>
                                                            {{$company_details_all->city}},
                                                            @if($company_details_all->state == null)

                                                            @else
                                                                {{$company_details_all->state}},
                                                            @endif
                                                            <br>{{$company_details_all->country}} - @if($company_details_all->pin == null)

                                                            @else
                                                                {{$company_details_all->pin}}
                                                            @endif
                                                        </p>
                                                        <ul class="">
                                                            @if($company_details_all->facebook == null)

                                                            @else
                                                                <li><a href="{{$company_details_all->facebook}}"><i class="fab fa-facebook"></i></a></li>
                                                            @endif
                                                            @if($company_details_all->twitter == null)

                                                            @else
                                                                <li><a href="{{$company_details_all->twitter}}"><i class="fab fa-twitter"></i></a></li>
                                                            @endif
                                                            @if($company_details_all->instagram == null)

                                                            @else
                                                                <li><a href="{{$company_details_all->instagram}}"><i class="fab fa-instagram"></i></a></li>
                                                            @endif
                                                            @if($company_details_all->youtube == null)

                                                            @else
                                                                <li><a href="{{$company_details_all->youtube}}"><i class="fab fa-youtube"></i></a></li>
                                                            @endif
                                                            @if($company_details_all->pinterest == null)

                                                            @else
                                                                <li><a href="{{$company_details_all->pinterest}}"><i class="fab fa-pinterest-p"></i></a></li>
                                                            @endif
                                                        </ul>
                                                    </div>
                                                </div>
                                    ...
                                    <div class="d-flex align-items-center">
                                        <input type="hidden" name="dep_val_{{$departure_details->id}}" id="dep-{{$departure_details->id}}-val" value="{{$departure_details->id}}">

                                        <a href="javascript:void(0);" class="pl-2 pr-2 chat_data tooltipbubble" id="dep-{{$departure_details->id}}" title="chat"><i class="far fa-comment-dots"></i></a>
                                        @if(isset($contact_person_det->mobile))
                                            <a href="tel:{{$contact_person_det->mobile}}" class="pl-2 pr-2 tooltipbubble" title="{{$contact_person_det->mobile}}"><i class="fas fa-phone fa-rotate-90 1"></i></a>
                                        @endif
                                        @if(isset($contact_person_det->email))
                                            <a href="mailto:{{$contact_person_det->email}}" class="pl-2 pr-2 tooltipbubble" title="{{$contact_person_det->email}}"><i class="far fa-envelope"></i></a>
                                        @endif
                                ...
                                @if($departure_details->ending_at != "")
                                    <div class="d-flex align-items-center">
                                        <i class="mdi mdi-map-marker-radius-outline mr-1"></i> <strong>Departure To : </strong> &nbsp; <strong class="text-uppercase text-primary">{{$departure_details->ending_at}}</strong>
                                    </div>
                                @endif
                                @if($departure_details->from != "")
                                    @if($departure_types->id != 4)
                                        <div class="d-flex align-items-center">
                                            <i class="mdi mdi-map-marker-multiple-outline mr-1"></i><strong> Ex : </strong> &nbsp
                                            <strong class="text-uppercase text-pink">{{$departure_details->from }}</strong>
                                        </div>
                                    @endif
                                @endif
                                ...
                                <div class="d-flex align-items-center">
                                    ...
                                    <strong class="text-uppercase ">{{date('d M, Y', strtotime($departure_details->start_date))}}</strong>
                                    <div class="ml-2 position-relative commingDates">
                                        ...
                                        <div class="alternateDateofPackage">
                                            ...
                                            <ul class="list-unstyled mb-0">
                                                @foreach($nearByDates as $nearbydate)
                                                    <li><a href="{{route('all_departure_details',$nearbydate->id)}}">{{date('d M, Y', strtotime($nearbydate->start_date))}} </a></li>
                                                @endforeach
                                            ...
                                            <div class="price-card">
                                                <div class="d-flex justify-content-between">
                                                    <div class="d-flex flex-column">
                                                        ...
                                                        <h4 class="price mb-0">Room: {{$hotel->total_room}}
                                                        </h4>
                                                    </div>
                                                </div>
                                                <hr class="mb-1 mt-1">
                                ...
                                @foreach($departure_prices as $price)
                                    <div class="col-md-3 ribbon-box">
                                        <div class="price-card" style="">
                                            <!-- <div class="ribbon-2 ribbon-2-primary"><span>{{ucfirst($price->sharing)}}</span></div> -->
                                            <div class="d-flex justify-content-between align-items-center">
                                                <div class="d-flex flex-column">
                                                    <h6 class="title">
                                                        @if(in_array(32, json_decode($columns)))
                                                            <span class="d-block">Sharing: <span class="text-info">{{ucfirst($price->sharing)}}</span></span>
                                                        @endif
                                                ...
                                                <div class="w-501 d-flex align-items-center justify-content-between flex-wrap">
                                                    ...
                                                    {{--<div class="transport">
                                                        <div class="dep-model-action-btn position-relative">
                                                            <a href="javascript:void(0);" data-toggle="modal" data-target=".bd-example-modal-sm " title="Hold Units" class="tooltipbubble"><i class="fas fa-pause"></i></a>
                                                            <a href="javascript:void(0);" data-toggle="modal" data-target=".bd-example-modal-sm469 " title="Book Units" class="tooltipbubble"><i class="far fa-calendar-check"></i></a>
                                                        </div>
                                                    </div>--}}
                                                    <div class="transport">
                                                        @if(in_array(33, json_decode($columns)))
                        ...
                        @endif
                        <!-- end row -->
                        @if(auth::user()->id == $departure_details->user_id || auth::user()->main_user_type == 2)
                            <div class="row">
                                    ...
                                    <table id="" class="table table-bordered table-striped">
                                        ...
                                        <tr>
                                          <td>{{$holding->departure_name}}</td>
                                              <td>{{$holding->name}}</td>
                                              <td>{{date('d M, Y h:i a', strtotime($holding->created_at."+5 hours +30 minutes"))}}</td>
                                              <td>{{$holding->booked_seat}}
                                        </td>
...
{{$hold_date->links()}}
                                ...
                                @endif
                                @if($departure_details->extra_hold_sum != 0)
                                    <!-- <div class="container table-bordered" >
                            <h5 class="card-user_name">Request More</h5>
                             ...
                             <div class="col-md-12">
                                <div class="table-responsive">
                                    <table id="" class="table table-bordered table-striped">
                                        <thead>
                                          ...
                                          <tbody>
                                          @foreach($extra_hold as $row)
                                        <tr>
                                           <td>{{$row->company_name}}</td>
                                               <td>{{$row->extra_hold_seat}} Unit</td>
                                               <td>{{date('d M, Y', strtotime("+{$row->date}"))}}</td>
                                               <td>
                                               <a href="" data-id="{{ $row->id }}" class="mr-2 ReleaseDepartue" title="Release"><i class="fa fa-unlink"></i></a>||
                                               <a href="{{url('/departures-hold-history-details/'.$row->id)}}" class="mr-2" title="More Details" style=><i class="fa fa-eye"></i></a>
                                               </td>
                                            </tr>
                                ...
                                @if(count($departure_book)> 0 )
                                    <!-- <div class="container table-bordered" >
                            <h5 class="card-title mb-2 mt-2">Booking Details <a href="{{route('all_departure_booking_history')}}" class="btn btn-info btn-sm pull-right">All Booking</a></h5>
                             <div class="col-md-12">
                                <div class="table-responsive">
                                   <table id="" class="table table-bordered table-striped">
                                        <thead >
                                        ...
                                        <tr>
                                          <td>{{$booking->departure_name}}</td>
                                              <td>{{$booking->name}}</td>
                                              <td>{{date('d M, Y h:i a', strtotime($booking->created_at."+5 hours +30 minutes"))}}</td>
                                              <td>{{$booking->booked_seat}}
                                        </td>
                                        <td>{{$booking->currency->currency_symbol}}   {{$booking->price}}
                                        </td>
                                        <td>
                                        <a href="{{url('/departures-booking-history-details/'.$booking->unique_id)}}" class="" title="More Details"><i class="fa fa-eye"></i></a>
                                              </td>
                                            </tr>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-12 d-flex align-items-center totalava_unit">
                                @if(($departure_details->total_seat)-($departure_details->hold_sum + $departure_details->book_sum)>0)
                                    <div class="bh_units">
                                        <input type="hidden" class="form-control" value="{{($departure_details->total_seat)-($departure_details->hold_sum + $departure_details->book_sum)}}" readonly>
                                        <div class="form-group mb-0">
                                            <h5 class="mb-0">Available Units :</h5>
                                            <span class="text-black text-bold">{{($departure_details->total_seat)-($departure_details->hold_sum + $departure_details->book_sum)}}</span>
                                            <input type="hidden" class="form-control" id="" name="available" value="{{($departure_details->total_seat)-($departure_details->hold_sum + $departure_details->book_sum)}}" name="available" readonly>
                                        </div>
                                    </div>
                                @else
                                    <div class="bh_units">
                                        ...
                                        <div class="form-group mb-0">
                                            <h5 class="mb-0">Available Units :</h5>
                                            <span class="text-black text-bold">{{($departure_details->total_seat)-($departure_details->hold_sum + $departure_details->book_sum)}}</span>
                                            <input type="hidden" class="form-control" id="" name="available" value="Over Holded:{{($departure_details->total_seat)-($departure_details->hold_sum + $departure_details->book_sum)}}" readonly>
                                        </div>
                                    </div>
                                @endif
                                <div class="bh_units">
                        ...
                        <div class="bookingMdodal">
                            <div class="row">
                                @foreach($departure_prices as $require)
                                    <input type="hidden" name="sairing[]" value="{{$require->sharing}}">
                                    <div class="row d-none">
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <input type="text" class="form-control" value="{{ucfirst($require->sharing)}}" readonly>
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        <input class="form-control required_unit{{$departure_details->id}}" id="require_hold_{{$require->id}}" name="hold[]">
                                    </div>
                                </div>-->
                                        <!--                                <div class="col-md-1" style="padding:0px">
                                    <div class="form-group">
                                        <input type="hidden" class="form-control" id="price_hold_{{$require->id}}" name="price[]" value="{{$require->price}}" style="border:none">
                                        <label id="require_hold_price_{{$require->id}}"></label>
                                        <input type="hidden" id="currency_c_hold_{{$require->id}}" value="{{$require->currency_code}} " name="currency">
                                        <input type="hidden" id="currency_hold_{{$require->id}}" value="{{$require->currency_symbol}} " name="currency_symbol">
                                    </div>
                                </div>-->
                                    </div>
                                        ...
                                        <div class="card">
                                            <div class="card-body">
                                                ...
                                                <div class="d-flex align-items-center justify-content-between">
                                                    ...
                                                    <div class="bh_units">
                                                        <input class="form-control required_unit{{$departure_details->id}}" id="require_hold_{{$require->id}}" name="hold[]" placeholder="Enter required units">
                                                    </div>
                                                </div>
                                                <hr class="mt-1 mb-1">
                                                <div class="d-flex align-items-center justify-content-between">
                ...
                <div class="modal-body">
                    <form role="form" id="BookDepartureForm">
                        ...
                        <div class="form-group d-none">
                            ...
                            <input type="text" class="form-control" id="" name="available" value="{{($departure_details->total_seat)-($departure_details->hold_sum + $departure_details->book_sum)}}" readonly>
                        </div>
                        <div class="form-group d-none">
                            <input type="hidden" name="id" value="{{$departure_details->id}}">
                            <div class="row">
                            ...
                            @foreach($departure_prices as $require)
                                <input type="hidden" name="sairing[]" value="{{ucfirst($require->sharing)}}">
                                <div class="row">
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <input type="text" class="form-control" id="" value="{{ucfirst($require->sharing)}}" readonly>
                                        </div>
                                    </div>
                                    <div class="col-md-2">
                                        <div class="form-group">
                        ...
                        <div class="bookingMdodal">
                            <div class="d-flex align-items-center totalava_unit">
                                ...
                                <span class="ml-2 text-black text-bold">{{($departure_details->total_seat)-($departure_details->hold_sum + $departure_details->book_sum)}}</span>
                            </div>
                            <div class="row">
                                @foreach($departure_prices as $require)
                                    <div class="col-md-6">
                                        <div class="card">
                                            <div class="card-body">
                                                ...
                                                <div class="d-flex align-items-center justify-content-between">
                                                    ...
                                                    <div class="bh_units">
                                                            <span>Hotel Name</span>
                                                            <strong>{{$require->hote_name}}</strong>
                                                        </div>
                                                        <div class="bh_units">
                                                            <span>Hotel Type</span>
                                                            <strong>{{$require->hotel_type}}</strong>
                        ...
                        <div class="modal-footer">
                            <div class="col-md-12 text-right">
                                <img src="{{ asset('images/loader.gif') }}" id="gif_book" style="width: 3%;  visibility: hidden;">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="modal-footer">
                            ...
                            <img src="{{ asset('images/loader.gif') }}" id="gif" style="width: 8%; display: none;">
...
@section('footerSection')
    <script>
        ...
        $("#require_{{$require->id}}").keydown(function () {
        ...
        $("#require_hold_{{$require->id}}").keyup(function () {
            ...
            if (required != '') {
                var total_required = required + required;
                var price = $("#price_hold_{{$require->id}}").val();
                var sum = parseInt(required) * parseInt(price);
                var currency = $("#currency_hold_{{$require->id}}").val();
                $("#require_hold_price_{{$require->id}}").html(currency + +sum);
                total = sum + total;
            } else {
                $("#require_hold_price_{{$require->id}}").html('');
            }
            $("#total_price_hold{{$row->id}}").html(total);
        })
    ...
    <script>
        $('#book').keyup(function () {
            var total = '{{($departure_details->total_seat)-($departure_details->hold_sum + $departure_details->book_sum)}}';
            if ($(this).val() > {{($departure_details->total_seat)-($departure_details->hold_sum + $departure_details->book_sum)}}) {
                //alert('Value can not be greater than {{($departure_details->total_seat)-($departure_details->hold_sum + $departure_details->book_sum)}}');
    ...
    <script>
        ...
        $("#hold{{$departure_details->id}}").keyup(function () {
            var required = $("#hold{{$departure_details->id}}").val();
            var price = ''
            var sum = parseInt(required) * parseInt(price);
