@extends('layouts.app')
@section('tagSection')
<title>Buyers List | Departure Cloud</title>
            ...
            <div class="widget-rounded-circle card-box bg-transparent px-0">
                <form class="" action="{{route('user_list')}}">
                    <div class="row m-md-0">
                        ...
                        <div class="col-md-3 col-sm-3 col-lg-3 p-md-0">
                            <div class="form-group mb-0">
                                <select class="form-control " name="status" id="name" data-toggle="select2">
                                    <option value="1" @if($statuss == 1) selected @endif>Verified</option>  
                                    <option value="2" @if($statuss == 2) selected @endif >Not Verified</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-md-2 col-sm-2 col-lg-2 p-md-0">
                            <a href="{{route('user_list')}}" class="btn btn-secondary w-100" style="border-radius: 0;"><i class="fas fa-times"></i> Clear</a>
                        </div>
                    </div>
                </form>
            </div>
                        ...
                        <div class="col-12 mb-2">
                            <nav>
                                <ul class="nav nav-tabs">
                                    <li class="nav-item"><a href="{{route('user_list')}}" class="nav-link active">Buyers</a></li>
                                    <li class="nav-item"><a href="{{route('suplier_list')}}" class="nav-link"> Suppliers </a></li>
                                    <li style="position:absolute;right:0px;right: 8px;top: -14px;">
                                ...
                                <table id="" class="table table-bordered table-striped">
                                      ...
                                      <tbody>
                                       ...
                                       <tr>
                                          ...
                                          <td style="color:#69204b;">@if($row->main_user_type == 0 && $row->role_id == 0)
                                                Buyer
                                            @endif
                                          </td>
                                          <td>{{date('d M, Y', strtotime($row->created_at))}}</td>
                                          <td><a class="badge badge-danger text-light font-weight-bold" href="{{ route('buyer_user_list', $row->id) }}" target="_blank" style="background-color: #2281f6;"><i class="fas fa-eye"></i></a>
                                          </td>
                                          ...
                                          <td>
                                            @if($row->verified==0)
                                               <span class="badge badge-danger text-uppercase font-weight-bold">UnApproved</span>
                                              @else
                                              <span class="badge badge-success text-uppercase font-weight-bold status12" data-id="{{ $row->tenant_id }}" data-status="{{ $row->varified }}">Approved</span>
                                              @endif
                                              <!-- @if($row->main_user_type == 0)
                                              <span class="badge badge-danger text-uppercase font-weight-bold"> Buyer</span>
                                              @endif -->
                                          </td>
                                          <td>
                                          @if($row->verified==0)
                                           <a class="badge badge-danger font-weight-bold text-light status1" data-id="{{ $row->tenant_id }}" data-status="{{ $row->varified }}" title="UnApproved"><i class="fas fa-check"></i></a>
                                          @endif
                                          @if($row->main_user_type == 0)
                                          <a class="badge badge-danger text-light font-weight-bold user-change" data-id="{{ $row->tenant_id }}" data-status="{{ $row->main_user_type }}" title="change user type"> <i class="fas fa-exchange-alt"></i></a>
                                          @endif
                                          </td>
                                       </tr>
                                       @endforeach
